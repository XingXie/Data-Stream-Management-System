/**
 * @file   nodes.h
 * @brief  nodes in the parse tree output by the parser, Modified by Xing
 */
...
typedef struct node {
	...
	union{    
		...
		// Single condition in where clause: boolean operation over two
		...
		// Window specification: window type.  Pointer to time-specification
		...
		// Attribute specification: name of the attribute, type and optionally length.
		struct {
			char         *attr_name;
			Type          type;
			unsigned int  len;
...
// WINDOW_SPEC Xing: Change row_win_node
NODE *time_win_node(NODE *time_spec);
NODE *row_win_node(int num_rows, int level);
NODE *now_win_node();
