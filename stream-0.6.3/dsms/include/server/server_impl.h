...
class ServerImpl : public Server {
 ...
 private:
	...
	bool completeShareFlag, subsumeShareFlag;

	// Xing: In subsume sharing, we have a couple of flags starting with ss
	bool ssWinSpec, ssSelect, ssProject;
	// Xing: For subsume sharing usage
	/*
	 * 1: winspec sharing
	 * 2: select sharing
	 * 3: project sharing
	...
	int ssCode;
	// 	index
	int logPlanBufferIndex;
	Logical::Operator logPlanBuffer[100];
 ...
 public:
	 ...
	 * @return            0 (success), !0 (otherwise)
	 */	
	int endAppSpecification(int serverId, Execution::Scheduler *trustedRR, int querylevel);
 ...
 private:
	...
	bool sharingAnalysis(Semantic::Query semQuery1, Semantic::Query semQuery2);

	// Xing: sharing timing
	ofstream myfile1;
	int startflag;
	struct timeval detail_time1, detail_time2; // added by Xing
	time_t now1, now2;
	struct tm *current1,  *current2;
	int msec1, msec2;

	// Xing: Buffer the logical plan
	void saveLogPlan(Logical::Operator logPlan);
};
