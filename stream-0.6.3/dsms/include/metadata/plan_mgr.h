...
namespace Metadata {
	...
	public:
		...
		// Xing: compelte share plan
		virtual int addCompleteSharePlan(unsigned int queryId, int completeShareID,
										Interface::QueryOutput *output) = 0;

		virtual int addSubsumeShareLogicalQueryPlan (unsigned int queryId,
												Logical::Operator *logPlan,
		...
		/**
		 * Initialize the scheduler with the instantiated operators.
		 * edited by Xing
		 */
		
		virtual int initScheduler (Execution::Scheduler *sched, int serverId, int querylevel) = 0;
