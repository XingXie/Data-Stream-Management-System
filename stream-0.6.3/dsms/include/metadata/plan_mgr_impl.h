...
namespace Metadata {
	...
	private:
		...
		/// For each query, we store the operator that produces the output
		/// of the query
		struct {
			unsigned int queryId;
			unsigned int opId;
			// Xing: store the plan node
	...
	public:
		 ...
		 */
		int addCompleteSharePlan(unsigned int queryId, int completeShareID, Interface::QueryOutput *output);

		/*
		 * Xing: add subsume share plan
		 */

		int addSubsumeShareLogicalQueryPlan (unsigned int queryId,
												Logical::Operator *logPlan,
																Interface::QueryOutput *output,
																int subsumeShareID,
																int ssCode);
		 ...
		 * This adds all the auxiliary (non-operator) structures like
		...
		//Xing edited
		int initScheduler (Execution::Scheduler *sched, int serverId, int querylevel);
	...
	private:
		...
		// Xing: generate share physical plan
		int genSubsumeSharePhysicalPlan(Logical::Operator *logPlan,
							Physical::Operator *&phyPlan,
							int ssCode);
		//----------------------------------------------------------------------
		//
