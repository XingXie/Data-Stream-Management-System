/**
 * @file             logop.h
 * @date             May 5, 2004 Modified by Xing:
...
namespace Logical {
	...
	struct Operator {		
		...
		union {
			...
			struct {
				...
				unsigned int level; // add by Xing
			} ROW_WIN;
	...
	// Windows 
	Operator *mk_row_window(Operator *input, unsigned int numRows, unsigned int level); // Edited by Xing
