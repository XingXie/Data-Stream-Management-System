...
class LogPlanGen {
 ...
 public:
	...
	// Xing: generate the subsume sharing logPlan
	int genSubsumeShareLogPlan (const Semantic::Query &query,
 ...
 private:
	...
	int genSubsumeSharePlan_n (const Semantic::Query &query,
				   Logical::Operator *&plan, int subsumeShareID, int ssCode);

	// Xing: no sharing op in subsume sharing methods
	int nsdoProjs_n (const Semantic::Query &query,
			Logical::Operator *inputPlan,
			Logical::Operator *&outputPlan);

	int nsapplyDistinct_n (const Semantic::Query &query,
			Logical::Operator *inputPlan,
			Logical::Operator *&outputPlan);

	int nsapplyR2SOps_n (const Semantic::Query &query,
			Logical::Operator *inputPlan,
			Logical::Operator *&outputPlan);
