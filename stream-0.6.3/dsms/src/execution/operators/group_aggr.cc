...
// Xing: The array keeps the LUB info. [0] is the current aggregation level, [1-4]
// are the counts of the tuple levels in the synopsis.
//unsigned int aggrLUBLevel[5] = {0};
// Xing: For minus output
int oldAggrLevel;
	...
	: LOG (_LOG)
{
	...
	this -> lastOutputTs        = 0;
	this -> bStalled            = false;
	// Xing: initialize the LUB level info
	this ->aggrLUBLevel[5]		= 0;
}
...
int GroupAggr::processPlus (Element inputElement)
{
	...
	// Xing: Currrent aggr level For minus usage. When a tuple expires from the window,
	// we should attach the LUB level with the expiring minus tuple
	oldAggrLevel = aggrLUBLevel[0];
	...
	if (bGroupExists) {
		...
		// Xing: Update the plus level
		//updatePlusLUBLevel(inputElement.level);
		aggrLUBLevel[1111 & inputElement.level]++;
		if (inputElement.level > aggrLUBLevel[0])
			aggrLUBLevel[0] = inputElement.level;
		...
		// Xing: Update level
		plusElement.level     = aggrLUBLevel[0];
		...
		// Xing: Use the old level
		minusElement.level     = oldAggrLevel;
...
int GroupAggr::processMinus (Element inputElement)
{
	...
	if (emptyGroupEval -> eval()) {
		...
		rc = outputSynopsis -> deleteTuple (oldAggrTuple);
		// Xing: Update the minus level
		// updateMinusLUBLevel(inputElement.level);
		aggrLUBLevel[1111 & inputElement.level]--;
			// From high to low level, if the count is not null, it is the LUB level
			// it should be stored in aggrLUBLevel[0]: from current LUB level
		// Only if the minus level equals the LUBlevel, we need to update
		if(inputElement.level == aggrLUBLevel[0] && aggrLUBLevel[inputElement.level] == 0){
			for(int k=aggrLUBLevel[0]-1; k>=1; k--){
				if(aggrLUBLevel[k]!=0){
				aggrLUBLevel[0] = k;
			//	printf("aggrLevel 1 %d: \n", aggrLUBLevel[1]);
			//	printf("Minus LUB current level %d: \n", aggrLUBLevel[0]);
				break;
				}
			}
		}
...
// Xing: update the LUB level group
//void GroupAggr::updatePlusLUBLevel(int level){
//	aggrLUBLevel[1111 & level]++;
	// From high to low level, if the count is not null, it is the LUB level
	// it should be stored in aggrLUBLevel[0]. Note that at least there are one tuple in that level
	//if(level > aggrLUBLevel[0])
	//	aggrLUBLevel[0] = level;
//}
// XingL update the aggrLevel array
/*void GroupAggr::updateMinusLUBLevel(int level){
	aggrLUBLevel[1111 & level]--;
	// From high to low level, if the count is not null, it is the LUB level
	// it should be stored in aggrLUBLevel[0]: from current LUB level
	for(int k=4; k>=1; k--){
...
*/
// Xing: get the current level
/*unsigned int GroupAggr::getLUBLevel(){
	return aggrLUBLevel[0];
}
*/
