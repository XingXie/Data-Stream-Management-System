...
{	
	...
	while (!outputQueue -> isFull() && innerScan -> getNext(innerTuple)) {
		...
		// Xing: process level info
		memcpy (&innerTupleLevel, innerTuple, INT_SIZE);
		// Xing: Level info is stored in element
		outputElement.level = ((innerTupleLevel >= outerElement.level) ? innerTupleLevel: outerElement.level);

	//	sprintf (LUBlevel, "%d", outputElement.level);
	//	strncat(outputTuple, LUBlevel, INT_SIZE);
		...
		// Xing: Test for level output
		//printf("outerplus element level: %d", outputElement.level);
...
int BinaryJoin::processOuterMinus (Element outerElement)
{
	...
	while (!outputQueue -> isFull() && innerScan -> getNext(innerTuple)) {
		...
		// Xing: Test for level output
		//printf("outerminus element level: %d", outputElement.level);
	}
...
{	
	...
	while (!outputQueue -> isFull() && outerScan -> getNext(outerTuple)) {
		...
		// Xing: process level info
		memcpy (&outerTupleLevel, outerTuple, INT_SIZE);
		// Xing: Level info is stored in element
		outputElement.level = ((outerTupleLevel>= innerElement.level)?outerTupleLevel:innerElement.level);

		outputQueue -> enqueue (outputElement);
		lastOutputTs = outputElement.timestamp;

		// Xing: Test for level output
		//printf("innererplus element level: %d", outputElement.level);
...
int BinaryJoin::processInnerMinus (Element innerElement)
{
	...
	// The output element
	Element outputElement;	// Added by Xing:
...
//added by Xing
//printf("I am in processinnerminus;\n");
	...
	while (!outputQueue -> isFull() && outerScan -> getNext(outerTuple)) {
		...
		// Xing: Test for level output
		//printf("innererminus element level: %d", outputElement.level);
	}
...
// Xing: compare the join tuple level with the input tuple level
/*int BinaryJoin::maxLevel(int a, int b)
{
	if(a>=b)
		return a;
	else
