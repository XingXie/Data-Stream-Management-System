...
int StreamSource::run (TimeSlice timeSlice)
{
	...
	for (unsigned int e = 0 ; e < numElements ; e++) {
		...
		// Xing: Get the level info
		memcpy(&level, inputTuple + offsets[0], INT_SIZE);
		// printf("get the level info in stream source: %d \n", level);

		// Get the attributes
	//	printf("stream_source.cc: numAttrs: %d \n", numAttrs);
		for (unsigned int a = 0 ; a < numAttrs ; a++) {
			switch (attrs [a].type) {				
			case INT:
		...
		// Xing: Pass level info
		outputQueue -> enqueue (Element(E_PLUS, outputTuple, inputTs, level));
