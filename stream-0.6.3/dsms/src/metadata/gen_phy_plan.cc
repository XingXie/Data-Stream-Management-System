...
// Xing: generate subsume share physical plan
int PlanManagerImpl::genSubsumeSharePhysicalPlan(Logical::Operator *logPlan,
									 Physical::Operator *&phyPlan,
									 int ssCode)
{
	int rc;
	...
	switch(ssCode){
	case 1:
		cout<< "gen_phy_plan.cc: it is winspec sharing" << endl;
		break;
	case 2:
		cout<< "gen_phy_plan.cc: it is select sharing" << endl;
		while(l_rootOp->kind!= Logical::LO_SELECT){
//		if(l_rootOp->kind == Logical::LO_SELECT){
//				cout << "we find one LO_SELECT" << endl;
//				scount ++;
//			}
			l_rootOp  = l_rootOp -> inputs [0];
		}
		cout << "gen_phy_plan.cc: found Logical::LO_SELECT" << endl;
		break;
	case 3:
	//	cout<< "gen_phy_plan.cc: it is project sharing" << endl;
		while(l_rootOp->kind!= Logical::LO_PROJECT){
			l_rootOp  = l_rootOp -> inputs [0];
		}
	...
	if(l_rootOp->kind == Logical::LO_SELECT){
		cout<< "We handle the LO_SELECT" << endl;
		if((rc = mk_select(l_rootOp, p_childPlans, phyPlan))!=0)
			return rc;
		cout<< "mk_select is done" << endl;
		l_rootOp = l_rootOp->output;
		rc = mk_project (l_rootOp, p_childPlans, phyPlan);
		cout << "mk_project return rc: " << rc << endl;
		return rc;
	}

	if(l_rootOp->kind == Logical::LO_PROJECT)
		return mk_project (l_rootOp, p_childPlans, phyPlan);

	// Never comes
		cout << "The logPlan.kind is wrong " << endl;
		return -1;
								 ...
								 Physical::Operator *&phyPlan)
{
	...
	row_win -> u.ROW_WIN.level = logPlan -> u.ROW_WIN.level; 	// Added by Xing
//	printf("In genphyplan level = %u \n"+row_win -> u.ROW_WIN.level); // Tested by Xing
	phyPlan = row_win;
