										  ...
										  Interface::QueryOutput *output)
{
	...
	//Xing: store the phyPlan for sharing purpose
	queryOutOps [numQueries].phyPlan = phyPlan;
	numQueries ++;
...
// Xing: Add subsume share plan
int PlanManagerImpl::addSubsumeShareLogicalQueryPlan (unsigned int queryId,
														Logical::Operator *logPlan,
														Interface::QueryOutput *output,
														int subsumeShareID,
														int ssCode)
{
	int rc;
	Physical::Operator *phyPlan, *rootOp, *sharedPhyPlan;
	Physical::Operator *outputOp;
	Physical::Operator *querySource;

	switch(ssCode){
	case 1:
		cout << "we share the winspec when generating the physical plan" << endl;
		break;
	case 2:
		cout << "we share the select when generating the physical plan" << endl;
		sharedPhyPlan = queryOutOps [subsumeShareID].phyPlan;
		while (sharedPhyPlan->kind != PO_SELECT){
			cout << "we miss the kind is: " << sharedPhyPlan->kind << endl;
			sharedPhyPlan = sharedPhyPlan->next;
		}
		cout << "we found PO_SELECT" << endl;
		phyPlan = sharedPhyPlan;

		break;
	case 3:
	//	cout << "we share the project when generating the physical plan" << endl;
		sharedPhyPlan = queryOutOps [subsumeShareID].phyPlan;
		while (sharedPhyPlan->kind != PO_PROJECT){
			cout << "we miss the kind is: " << sharedPhyPlan->kind << endl;
			sharedPhyPlan = sharedPhyPlan->next;
		}
		cout << "we found PO_PROJECT" << endl;
...
/*		while (sharedPhyPlan->kind != PO_PROJECT){
		...
		break;
	default:
		cout << "plan_mgr_impl.cc: wrong ssCode" << endl;
		break;
	}
	...
	// generate the physical plan for the query
	if ((rc = genSubsumeSharePhysicalPlan (logPlan, phyPlan, ssCode)) != 0)
		return rc;
...
//Xing: add complete share plan
int PlanManagerImpl::addCompleteSharePlan (unsigned int queryId, int completeShareID, Interface::QueryOutput *output)
{
	int rc;
	Physical::Operator *outputOp;
	...
	// Save query Id
	queryOutOps [numQueries].queryId = queryId;
	// Find the sharing query
	queryOutOps [numQueries].opId = queryOutOps [completeShareID].opId;
	queryOutOps [numQueries].phyPlan = queryOutOps [completeShareID].phyPlan;
	...
	if (output) {
		...
		// Create an output operator that reads from the query
		if ((rc = mk_output (queryOutOps [numQueries].phyPlan, outputOp)) != 0)
			return rc;
									 ...
									 unsigned int schemaBufLen)
{
	...
	// Find the output operator for that query
	op = (ops + outOpId);
	...
	if (nused == schemaBufLen - 1)
		return -1;
	// Reduce the schema buffer by the material adding
	schemaBufLen -= nused;
	ptr += nused;
	
	// Encode the columns
	//printf("in planmgr.gerQeurySchema of op.numAttrs: %d \n", op->numAttrs);
	for (unsigned int a = 0 ; a < op -> numAttrs ; a++) {
	...
	/*	case LUB:
			...
			printf("I am adding LUB with column =%d \n", a+1);
			nused = snprintf (ptr, schemaBufLen,
							  "<column type = \"byte\"/>\n");
			if (nused == schemaBufLen - 1)
				return -1;
...
int PlanManagerImpl::map (unsigned int queryId, unsigned int tableId)
{
	...
	// We did not find an output operator for this query. (this should
...
//	cout<< "plan_mgr_impl.cc: enter inst_mem"  << endl;

	if ((rc = inst_mem_mgr ()) != 0)
		return rc;	
//	cout<< "plan_mgr_impl.cc: enter inst_static"  << endl;
	if ((rc = inst_static_tuple_alloc ()) != 0)
		return rc;
//	cout<< "plan_mgr_impl.cc: enter inst_ops"  << endl;
	if ((rc = inst_ops ()) != 0)
		return rc;
//	cout<< "plan_mgr_impl.cc: enter inst_queue"  << endl;
	if ((rc = inst_queues ()) != 0)
		return rc;
//	cout<< "plan_mgr_impl.cc: enter link_syn_store"  << endl;
	if ((rc = link_syn_store ()) != 0)
		return rc;
//	cout<< "plan_mgr_impl.cc: enter link_in_store"  << endl;
...
int PlanManagerImpl::initScheduler (Execution::Scheduler *sched, int serverId, int querylevel)
{
	int rc;
	Operator *op;
 ...
 while (op) {
		...
		if ((rc = sched->addOperator(op -> instOp, serverId, querylevel)) != 0)
			return rc;
