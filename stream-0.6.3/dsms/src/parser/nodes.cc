...
*/
// Xing: The level should be saved in row.
NODE *row_win_node(int num_rows, int level) {
	NODE *n = newnode(N_WINDOW_SPEC);
  
	n -> u.WINDOW_SPEC.type = ROW;
	n -> u.WINDOW_SPEC.num_rows = num_rows;
	n -> u.WINDOW_SPEC.level = level;
//	 printf("The nodes.cc level: %d \n", n -> u.WINDOW_SPEC.level );
	n -> u.WINDOW_SPEC.time_spec = 0;
	n -> u.WINDOW_SPEC.part_win_attr_list = 0;
