...
int LogPlanGen::genSubsumeShareLogPlan (const Semantic::Query &query,
							Logical::Operator *& queryPlan, int subsumeShareID, int ssCode)
{
	...
	// Generate naive plan
	if ((rc = genSubsumeSharePlan_n (query, queryPlan, subsumeShareID, ssCode)) != 0)
		return rc;

//	cout << "log_plan_gen.cc: start checking plan" << endl;
	...
	if((rc = t_rstreamNow (queryPlan)) != 0)
		return rc;
//	cout << "log_plan_gen.cc: finish rstreamNow" << endl;

	ASSERT (check_plan (queryPlan));

	if((rc = t_removeIstream(queryPlan)) != 0)
		return rc;
//	cout << "log_plan_gen.cc: finish rsIstream" << endl;

	ASSERT (check_plan (queryPlan));

	if((rc = t_streamCross (queryPlan)) != 0)
		return rc;
//	cout << "log_plan_gen.cc: finish rstreamCross" << endl;
...
//	cout << "log_plan_gen.cc: finish removeProject" << endl;

	ASSERT (check_plan (queryPlan));

//	if((rc = t_makeCrossBinary (queryPlan)) != 0)
//		return rc;
//	cout << "log_plan_gen.cc: finish t_makeCrossBinary" << endl;

	ASSERT (check_plan (queryPlan));

//	if((rc = t_makeStreamCrossBinary (queryPlan)) != 0)
//		return rc;
//	cout << "log_plan_gen.cc: finish t_makeStreamCrossBinary" << endl;

	ASSERT (check_plan (queryPlan));

	if((rc = t_pushSelect(queryPlan)) != 0)
		return rc;
//	cout << "log_plan_gen.cc: finish pushSelect" << endl;

	ASSERT (check_plan (queryPlan));

//	cout << "log_plan_gen.cc: leaving" << endl;
