...
// Row window
Operator *Logical::mk_row_window(Operator *input, unsigned int numRows, unsigned int level) // Edited by Xing
{
	...
	op -> u.ROW_WIN.level = level; // Added by Xing
//	printf("In logopcc mke_row_window level = %d \n"+ op -> u.ROW_WIN.level); // Tested by Xing
