...
// Xing: Buffer each operator of plan

Operator   *joinBuffer[100];
Operator   *selectBuffer[100];
Operator   *aggrBuffer[100];
Operator   *projectBuffer[100];
Operator   *distinctBuffer[100];
Operator   *r2sBuffer[100];
...
// Xing: subsume sharing plan
// Xing: generate subsume plan
int LogPlanGen::genSubsumeSharePlan_n (const Semantic::Query &query,
							Logical::Operator *& queryPlan, int subsumeShareID, int ssCode)
{
	int rc;
	...
	switch(ssCode){
	case 1:
		cout << "sharing the winspec" << endl;
		break;
	case 2:
		cout << "log_naive_plan_gen.cc: sharing the select" << endl;
		cout << "log_naive_plan_gen.cc: current bufferIndex: " << bufferIndex << endl;

		joinBuffer[bufferIndex] = joinBuffer[subsumeShareID];
		queryPlan = selectBuffer[subsumeShareID];

		cout<< "log_naive_plan_gen.cc: query.numPreds " << query.numPreds << endl;


		// Apply WHERE clause predicates over the join
		if ((rc = applyPreds_n (query, queryPlan, select)) != 0)
			return rc;
	...
	//	select = queryPlan;
		...
		cout << "log_naive_plan_gen.cc: applyPreds_n is done" << endl;
		// Xing: buffer the operator
		// IMPORTANT: We only buffer the last selection op!
		selectBuffer[bufferIndex] = select;

		// Apply Aggregations & perform group by if necessary
		if ((rc = applyAggr_n (query, select, aggr)) != 0)
			return rc;
		cout << "log_naive_plan_gen.cc: applyAggr_n is done" << endl;
		// Xing: buffer the operator
		aggrBuffer[bufferIndex] = aggr;
	//	tempOp = select;
		// Perform projections specified in SELECT clause
		// Xing: LUB
		if ((rc = doProjs_n (query, aggr, project)) != 0)
			return rc;
		cout << "log_naive_plan_gen.cc: doProjs_n is done" << endl;
		// Xing: buffer the operator
		projectBuffer[bufferIndex] = project;

		// Apply Distinct operator if needed
		if ((rc = applyDistinct_n (query, project, distinct)) != 0)
			return rc;
		cout << "log_naive_plan_gen.cc: applyDistinct_n is done" << endl;
		// Xing: buffer the operator
		distinctBuffer[bufferIndex] = distinct;

		// Apply R2S operators if present ...
		if ((rc = applyR2SOps_n (query, distinct, r2s)) != 0)
			return rc;
		cout << "log_naive_plan_gen.cc: applyR2SOps_n is done" << endl;

		// Xing: buffer the operator
		r2sBuffer[bufferIndex] = r2s;
		...
		break;
	case 3:
//		cout << "log_naive_plan_gen.cc:sharing the project" << endl;

		joinBuffer[bufferIndex] = joinBuffer[subsumeShareID];
		selectBuffer[bufferIndex] = selectBuffer[subsumeShareID];
		aggrBuffer[bufferIndex] = aggrBuffer[subsumeShareID];

		queryPlan = projectBuffer[subsumeShareID];

		// The non-sharing operators
		if ((doProjs_n (query, queryPlan, project)) != 0)
				return rc;
	...
	bufferIndex ++;
//	cout << "leaving log_naive_plan_gen.cc" << endl;
	return 0;
}
						  ...
						  Operator *&outputPlan) 
{
	...
	case ROW:
		...
		winOp = mk_row_window (inputStr, win.u.ROW.numRows, win.u.ROW.level); //edited by Xing
		break;
...
// Xing: nosharing op in subsume plan generation
int LogPlanGen::nsdoProjs_n (const Semantic::Query &query,
					  Operator *inputPlan,
					  Operator *&outputPlan) {
	Operator      *project;
	unsigned int   numProjExprs;
...
int LogPlanGen::nsapplyDistinct_n (const Semantic::Query &query,
							Operator *inputPlan,
							Operator *&outputPlan)
{
	if (query.bDistinct)
...
int LogPlanGen::nsapplyR2SOps_n(const Semantic::Query &query,
						  Operator *inputPlan,
						  Operator *&outputPlan)
{
	// Query does not have an R2S operator
