/**
 * @file            sem_interp.cc
 * @date            May 11, 2004, Modified by Xing
...
// Xing: Set and Get LUB flags
/*
void SemanticInterpreter::setIsStar(){
	isStar = true;
}
 void SemanticInterpreter::setIsJoin(){
	isJoin = true;
}
*/
/**
 ...
 *
 * @return            0 (success), !0 (failure)       
 * Xing: We add LUB level here for all the queries
 */
 ...
 * Xing: We need to handle LUB here
 */
 ...
 * form.  Hardly anything to do ... Xing: level is added.
 */
static int interpretRowWin(NODE *synWin, WindowSpec& semWin)
{
	semWin.type          = ROW;
	semWin.u.ROW.numRows = synWin -> u.WINDOW_SPEC.num_rows;
	semWin.u.ROW.level   = synWin -> u.WINDOW_SPEC.level; // Added by Xing
//	printf("In seminterpretRowWin: %u \n" + semWin.u.ROW.level); // Test by Xing
	return 0;
}
...
static int interpretFromClause(NODE *fromClause, Query& query)
{
	...
	// Xing: more than one table is join
//	int count = 0;
 ...
 * Xing: We handle LUB signal here
 */
static int interpretStarInSelect(Query& query) {
	...
	numProjExprs = 0;
	// Xing:
	//printf("In sem_interp:interpretStarInSelect \n");
	...
	expr -> exprType      = E_ATTR_REF;
	expr -> type          = INT;
	// Xing: the varId needs reconsider
	expr -> u.attr.varId  = varId;
	expr -> u.attr.attrId = numProjExprs;

	query.projExprs [ numProjExprs ] = expr;
	numProjExprs ++;
//	printf("number of numProjExprs: %d \n", numProjExprs);
	// --------------
	...
	// Xing: 99 means we need to set isStar as true
	return 0;
}
...
static int interpretSelectClause (NODE *selectClause, Query& query)
{
	...
	expr -> exprType      = E_ATTR_REF;
	expr -> type          = INT;
	// Xing: the varId needs reconsider: Here it is only one table!
	//printf("sem_intep.cc: varId %d \n", varId);
	//printf("sem_intep.cc: numProjExprs %d \n", numProjExprs);
	expr -> u.attr.varId  = query.fromClauseTables[0];
	expr -> u.attr.attrId = numProjExprs;

	query.projExprs [ numProjExprs ] = expr;
	numProjExprs ++;
//	printf("in explicit projections: number of numProjExprs: %d \n", numProjExprs);
	// --------------
...
{	
	...
	// Get the list of tables referenced by this query
	rc = getReferencedTables(parseTree, query);
	if(rc != 0)
		return rc;	
	...
	if (query.kind == SFW_QUERY) {
		...
		rc = interpretFromClause(sfw_block -> u.SFW_BLOCK.rel_list,
											 query);
		if(rc != 0)
			return rc;
		...
		// Xing: We add LUB attribute
		rc = interpretSelectClause(sfw_block -> u.SFW_BLOCK.select_clause,
													   query);
		if(rc != 0)
			return rc;
	}
...
/* Xing: not needed
bool SemanticInterpreter::checkIsStar(){
	return isStar;
}
bool SemanticInterpreter::checkIsJoin(){
	return isJoin;
}
*/
