...
namespace Network {
	...
	private:
		...
		/// Indicator for the end of the output stream: move to public for DIS
	//	bool bEnd;
	...
	private:
		...
		bool buf_full () const;
//		int writeln (char *line); MOVE TO PUBLIC FOR DIS
		int writechar (char c);
	};

class ThreadDist : public Thread {
public:
		...
		ThreadDist(payload *p, OutputConnection *outCon, ostream &_LOG);
	//	ThreadDist(const char* ipaddress, const char* port, int outputId, OutputConnection *outCon);
	//	ThreadDist(const char* ipaddress, const char* port, int outputId, OutputConnection *outCon, std::ostream &_LOG);
	//	char ip_Address[50];
	//	char portNo[10];
	//	int output_id;

		payload *pLoad;
		OutputConnection *outCon;
