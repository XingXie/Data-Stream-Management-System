...
namespace Network {
	...
	class InputConnection : public Thread, public Interface::TableSource {		
		...
		bool master;
		//INDEX of the next slave to be send data
		int nextSlaveIndex;
	...
	public:
		...
		InputConnection (ostream& LOG, string userLevel);
		...
		// DIS
		int distributeLoad (char *distributeReadBuf, int bufLen, int inSocketId);///Philips
	...
	private:
		...
		int loadBuf ();
		// MLS
		int securitySourceManagement(char *lineBuf);

		//DIS---
		int getSchemaBuff (char *lineBuf, int lineBufSize);
		void incrementNextSlaveIndex();
		//
	};
}
#endif
