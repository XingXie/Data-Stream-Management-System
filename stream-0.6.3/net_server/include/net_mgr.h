/**
 *
 * Xing 2010.10.20: MLS control are added.
 */
#ifndef _NET_MGR_
#define _NET_MGR_
...
namespace Network {
	...
	private:
		...
		// DIS
		static vector<payload> ip_table;
		//----------------

	public:
		//Xing DIS
		unsigned int serverLevel;
		 ...
		 */
		 ...
		 //added by Xing
				Execution::RoundRobinScheduler *trustedRR;
		//Xing: Server Level
		NetworkManager (int portNo, char *logFilePref, char *configFile, int serverLevel);
		 ...
		 * @param userLevel the user level who issued the query
		 */
		
		int createInputConn (int &conn_id, InputConnection *&conn, string userLevel);
		 ...
		 * command connection returns the identifier of the output
		...
		// DIS
		static void* basicgetscore_starter(void *p) {
			NetworkManager *t = (NetworkManager *)p;
			t->getServerInfo();
		}

		char* getLoad();
		int strpos(char *haystack, char *needle);
		char * getModel(int para);
		double string_to_double( const std::string& s );
		int calculate(std::string myString);
		char* getInterfaceAddress();
		void broadcastIP();
		void setupListener();
		std::string getIP(std::string myString);
		void* getServerInfo();

		static vector<payload>* getIpTable() {
			return &ip_table;
		}
