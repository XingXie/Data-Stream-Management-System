...
import stream.vis.util.NonFatalException;
import stream.vis.util.Error;
...
public class Client {
    ...
    public void setServerInfo (String host, int port, String user, String pwd) throws FatalException {
	assert (state == INIT);
	// Authentication is performed when setting up connection
	serverConn = new ServerConnection (host, port, user, pwd);	
	
	BeginAppRet ret = serverConn.beginApp ();
	// Xing test
	//System.out.println("In client, can beginApp method.");
	if (ret.errorCode != 0) {
    ...
    /// Get plan
    public void genPlan () throws FatalException {
	assert (state == USERAPPSPEC);
